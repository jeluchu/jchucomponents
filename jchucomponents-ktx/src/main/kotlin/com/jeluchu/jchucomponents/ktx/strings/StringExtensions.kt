...
fun String.getBitmapFromURL(): Bitmap? = networkBitmapFromUrl()

fun String.toBitmapDrawable(context: Context): BitmapDrawable = BitmapDrawable(context.resources, getBitmapFromURL())
...
fun String.isNumber() = String.empty() != this && Pattern.compile("^[0-9]*$").matcher(this).matches()

fun String.isEmail(): Boolean = String.empty() != this && Patterns.EMAIL_ADDRESS.matcher(this).matches()

fun String.isPassword() = String.empty() != this && Pattern.compile(PASSWORD_REGEX).matcher(this).matches()

fun String.isValidPassword() = String.empty() != this && Pattern.compile(VALID_PASSWORD_REGEX).matcher(this).matches()

fun String.isValidPhone() = String.empty() != this && Pattern.compile(VALID_PHONE_REGEX).matcher(this).matches()
...
fun String.capitalizeWords(): String = split(" ").joinToString(" ") { it.uppercase(Locale.getDefault()) }

fun String.capitalizeFirstLetter(): String = substring(0, 1).uppercase(Locale.getDefault()) + this.substring(1)

fun String.remove(
    value: String,
    ignoreCase: Boolean = false
): String = replace(value, String.empty(), ignoreCase)

fun String.remove(
    @RegExp pattern: String
) = remove(Regex(pattern, RegexOption.IGNORE_CASE))
