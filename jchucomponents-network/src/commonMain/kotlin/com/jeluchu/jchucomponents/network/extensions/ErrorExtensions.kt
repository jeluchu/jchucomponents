...
fun Throwable.toFailure(): Failure = when (this) {
    ...
    is ResponseException -> Failure.ServerError(
        statusCode = com.jeluchu.jchucomponents.network.http.getHttpErrorInfo(response.status.value),
...
fun Failure?.handleFailure(): String = when (this) {
    ...
    is Failure.Timeout,
    is Failure.UnknownError -> message
