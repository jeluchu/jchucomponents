...
import com.jeluchu.qr.FormatException.Companion.formatInstance
...
class Version private constructor(
    ...
    private val ecBlocks: MutableList<ECBlocks> = ecBlocks.toMutableList()
    val totalCodewords: Int
    val dimensionForVersion: Int
        get() = 17 + 4 * versionNumber

    fun getECBlocksForLevel(ecLevel: ErrorCorrectionLevel): ECBlocks = ecBlocks[ecLevel.ordinal]

    /**
     * See ISO 18004:2006 Annex E
     */
    fun buildFunctionPattern(): BitMatrix {
        val dimension = dimensionForVersion
        val bitMatrix = BitMatrix(dimension)
        ...
        // Alignment patterns
        val max = alignmentPatternCenters.size
        for (x in 0 until max) {
            val i = alignmentPatternCenters[x] - 2
     ...
     */
    class ECBlocks internal constructor(
        val eCCodewordsPerBlock: Int,
        vararg ecBlocks: ECB
    ) {
        val eCBlocks: MutableList<ECB> = ecBlocks.toMutableList()
        val numBlocks: Int
            get() {
                var total = 0
     ...
     */
    class ECB internal constructor(
        val count: Int,
    ...
    companion object {
         ...
         */
        private val VERSION_DECODE_INFO =
            intArrayOf(
                0x07C94,
                0x085BC,
                0x09A99,
        ...
        fun decodeVersionInformation(versionBits: Int): Version? {
            var bestDifference = Int.MAX_VALUE
            var bestVersion = 0
            for (i in VERSION_DECODE_INFO.indices) {
                val targetVersion = VERSION_DECODE_INFO[i]
                ...
                // we have seen so far
                val bitsDifference = FormatInformation.numBitsDiffering(versionBits, targetVersion)
